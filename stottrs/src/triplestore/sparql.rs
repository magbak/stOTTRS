...
use crate::triplestore::sparql::lazy_aggregate::sparql_aggregate_expression_as_lazy_column_and_expression;
use crate::triplestore::sparql::lazy_expressions::lazy_expression;
use crate::triplestore::sparql::lazy_order::lazy_order_expression;
use crate::triplestore::sparql::lazy_triple::lazy_triple_pattern;
...
impl Triplestore {
    pub fn combine_static_and_time_series_results(
        ...
        lf = self.lazy_graph_pattern(&mut columns, lf, inner_graph_pattern,  &context);
    ...
    fn lazy_graph_pattern(
        ...
        match graph_pattern {
            GraphPattern::Bgp { patterns } => {
                ...
                for p in patterns {
                    output_lf =
                        lazy_triple_pattern(columns, output_lf, p,  &bgp_context);
                }
                output_lf
            }
            GraphPattern::Path { .. } => {
            ...
            GraphPattern::LeftJoin {
                ...
                if let Some(expr) = expression {
                    ...
                    right_lf =
                        lazy_expression(expr, right_lf, columns,  &expression_context);
                    right_lf = right_lf
            ...
            GraphPattern::Filter { expr, inner } => {
                ...
                inner_lf =
                    lazy_expression(expr, inner_lf, columns,  &expression_context);
                inner_lf = inner_lf
                    .filter(col(&expression_context.as_str()))
                    .drop_columns([&expression_context.as_str()]);
                inner_lf
            ...
            GraphPattern::Extend {
                ...
                if !columns.contains(variable.as_str()) {
                    inner_lf =
                        lazy_expression(expression, inner_lf, columns,  &inner_context)
