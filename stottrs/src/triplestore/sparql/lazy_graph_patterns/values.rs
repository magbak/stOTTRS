...
impl Triplestore {
    pub(crate) fn lazy_values(&self, solution_mappings:Option<SolutionMappings>, variables:&Vec<Variable>, bindings:&Vec<Vec<Option<GroundTerm>>>, context:&Context) -> Result<SolutionMappings, SparqlError> {
                ...
                if let Some(gt) = col {
                    ...
                    match gt {
                        GroundTerm::NamedNode(nn) => {
                            ...
                            col_vecs.get_mut(&j).unwrap().push(AnyValue::Utf8Owned(nn.as_str().into()));
                        }
                        GroundTerm::Literal(lit) => {
                            ...
                            if i == 0 {
                                ...
                                match existing {
                                    ...
                                    _ => {todo!("No support for values of some varibales having different types")}
                                }
                            }
        ...
        if let Some(mut mappings) = solution_mappings {
            let join_on:Vec<String> = variables.iter().map(|x|x.as_str().to_string()).filter(|v|mappings.columns.contains(v)).collect();
